...
Flows exported via ZMQ by nProbe™ Cento are encrypted using CURVE encryption. The <pub key> is the public key generated by ntopng itself when started with :code:`--zmq-encryption` and shown on its ZMQ interface status page.

If this option is not specified, nProbe™ Cento falls back to a public, built-in, default key so that flows are still encrypted rather than sent in cleartext. This default key only prevents accidental cleartext exposure on the network and offers no real confidentiality, since it is publicly known and shared by every Cento/ntopng installation that has not configured a dedicated key: it must be replaced by configuring a dedicated key pair for any deployment that requires actual security. See the `ntopng documentation <https://www.ntop.org/guides/ntopng/using_with_other_tools/cento.html>`_ for a complete description of how to configure ZMQ encryption between Cento and ntopng.
