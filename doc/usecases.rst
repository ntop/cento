...
Flows exported over ZMQ are encrypted by default: if no dedicated key is configured via *--zmq-encryption-key*, nProbe™ Cento falls back to a public, built-in, default key shared by every Cento/ntopng installation, which only prevents accidental cleartext exposure. A dedicated key pair, generated by ntopng, should be configured for any deployment that requires real confidentiality. See the *--zmq-encryption-key* option description in the Options section, and the `ntopng documentation <https://www.ntop.org/guides/ntopng/using_with_other_tools/cento.html>`_ for details on configuring ZMQ encryption.
